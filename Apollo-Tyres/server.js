...
// ADD THIS NEW ENDPOINT after /api/store-excel-data (around line 991)
app.post('/api/store-project-matrix', async (req, res) => {
  try {
    ...
    await db.query(`DELETE FROM ${projectTable} WHERE project_id = $1`, [projectId]);

    // Copy all rows from scratch table to project table
    let insertQuery;
    ...
    if (protocol === 'MF62') {
      insertQuery = `
        ...
        SELECT $1, number_of_runs, tests, ips, loads, inclination_angle, slip_angle, slip_ratio, test_velocity, job, old_job, template_tydex, tydex_name, p, l
        FROM ${scratchTable}
      `;
      ...
      insertQuery = `
        ...
        SELECT $1, number_of_runs, tests, inflation_pressure, loads, inclination_angle, slip_angle, slip_ratio, test_velocity, job, old_job, template_tydex, tydex_name, p, l
        FROM ${scratchTable}
      `;
      ...
      insertQuery = `
        ...
        SELECT $1, number_of_runs, tests, loads, inflation_pressure, test_velocity, longitudinal_slip, slip_angle, inclination_angle, cleat_orientation, job, old_job, template_tydex, tydex_name, p, l
        FROM ${scratchTable}
      `;
    } else if (protocol === 'CDTire') {
      insertQuery = `
        INSERT INTO ${projectTable}
        (project_id, number_of_runs, test_name, inflation_pressure, velocity, preload, camber, slip_angle, displacement, slip_range, cleat, road_surface, job, old_job, template_tydex, tydex_name, p, l)
        SELECT $1, number_of_runs, test_name, inflation_pressure, velocity, preload, camber, slip_angle, displacement, slip_range, cleat, road_surface, job, old_job, template_tydex, tydex_name, p, l
        FROM ${scratchTable}
      `;
      ...
      insertQuery = `
        ...
        SELECT $1, number_of_runs, tests, inflation_pressure, loads, inclination_angle, slip_angle, slip_ratio, test_velocity, cleat_orientation, displacement, job, old_job, template_tydex, tydex_name, p, l
        FROM ${scratchTable}
      `;
    }

    await db.query(insertQuery, [projectId]);

    res.json({ success: true, message: 'Matrix data saved to project table' });
    ...
    function executeAbaqusJob(folderPath, jobName, oldJobName, folderName = '') {
        return new Promise((resolve) => {
            try {
                ...
                // Broadcast run start
                broadcastSse('run-status', { run: cleanJobName, folder: folderName, status: 'running', progress: 0 });
                ...
                abaqusProcess.on('close', (code) => {
                    ...
                    if (code === 0) {
                        ...
                        broadcastSse('run-status', { run: cleanJobName, folder: folderName, status: 'done', progress: 100 });
                        resolve({ success: true, output: output });
                    } else {
                        // Broadcast run failure
                        broadcastSse('run-status', { run: cleanJobName, folder: folderName, status: 'failed', message: errorOutput || output, progress: 100 });
                        resolve({ success: false, message: `Process exited with code ${code} in folder ${folderName}`, error: errorOutput, output: output });
                    }
                });
...
// GET /api/projects - List all projects (with optional filtering by user)
app.get('/api/projects', authenticateToken, async (req, res) => {
  try {
    ...
    if (userRole === 'manager') {
      query = `
        SELECT id, project_name, region, department, tyre_size, protocol, 
               status, created_at, completed_at, user_email
        FROM projects 
        ORDER BY created_at DESC
      `;
      params = [];
    ...
    console.log(`✅ Fetched ${result.rows.length} projects for user: ${userEmail}`);
    
    // Return array directly (my-projects.js expects an array)
    res.json(result.rows || []);
...
app.patch('/api/projects/:id/archive', authenticateToken, async (req, res) => {
  try {
    ...
    const updateQuery = 'UPDATE projects SET status = $1 WHERE id = $2 RETURNING *';
...
app.patch('/api/projects/:id/unarchive', authenticateToken, async (req, res) => {
  try {
    ...
    const updateQuery = 'UPDATE projects SET status = $1 WHERE id = $2 RETURNING *';
    const updateResult = await db.query(updateQuery, ['Not Started', projectId]);
...
// Mark Project as In Progress (NEW ENDPOINT - Add this)
